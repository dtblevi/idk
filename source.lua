--[[
	...
	Opiam Interface Suite
	by ChronicTemptations

	ChronicTemptations  | Designing + Programming
	ChronicTemptations   | Programming
	ChronicTemptations    | Programming
	ChronicTemptations | Programming
...
local requestsDisabled = true --getgenv and getgenv().DISABLE_Opiam_REQUESTS
local InterfaceBuild = '3K3W'
local Release = "Build 1.68"
local OpiamFolder = "Opiam"
local ConfigurationFolder = OpiamFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		OpiamOpen = {Type = 'bind', Value = 'K', Name = 'Opiam Keybind'},
		-- buildwarnings
		-- Opiamprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.opiamOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Opiam | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, OpiamFolder) then
				if callSafely(isfile, OpiamFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, OpiamFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"OpiamdOpen":{"Value":"K","Type":"bind","Name":"Opiam Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Opiam Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
			...
			-- If no settings saved, apply overridden settings only
				...
				for settingName, settingValue in overriddenSettings do
					...
					assert(#split == 2, "Opiam | Invalid overridden setting name: " .. settingName)
	...
	if not success then 
		if writefile then
			warn('Opiam had an issue accessing configuration saving capability.')
		end
	end
end
...
local Opiam = useStudio and script.Parent:FindFirstChild('Opiam') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local OpiamDestroyed = false -- True when OpiamLibrary:Destroy() is called

repeat
	if Opiam:FindFirstChild('Build') and Opiam.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Opiam | Build Mismatch')
		print('Opiam may encounter issues as you are running an incompatible interface version ('.. ((Opiam:FindFirstChild('Build') and Opiam.Build.Value) or 'No Build') ..').\n\nThis version of Opiam is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Opiam = Opiam, useStudio and script.Parent:FindFirstChild('Opiam') or game:GetObjects("rbxassetid://10804731440")[1]
...
elseif syn and syn.protect_gui then 
	syn.protect_gui(Opiam)
	Opiam.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Opiam.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Opiam.Name and Interface ~= Opiam then
			Interface.Enabled = false
			Interface.Name = "Opiam-Old"
		end
	end
elseif not useStudio then
...
if Opiam.AbsoluteSize.X < minSize.X and Opiam.AbsoluteSize.Y < minSize.Y then
...
local Main = Opiam.Main
local MPrompt = Opiam:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Opiam:FindFirstChild('Drag')
...
Opiam.DisplayOrder = 100
LoadingFrame.Version.Text = Release

-- Thanks to Latte Softworks for the Lucide integration for Roblox
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Opiam/refs/heads/main/icons.lua')
-- Variables
...
local Notifications = Opiam.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when Opiam is destroyed

local SelectedTheme = OpiamLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = OpiamLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Opiam.Main.BackgroundColor3 = SelectedTheme.Background
	Opiam.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Opiam.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Opiam.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Opiam.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Opiam.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Opiam.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Opiam.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Opiam.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Opiam:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Opiam | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Opiam | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Opiam had an issue decoding the configuration file, please try delete the file and reopen Opiam.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(OpiamLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Opiam | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--OpiamLibrary:Notify({Title = "Opiam Flags", Content = "Opiam was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(OpiamLibrary.Flags) do
...
function OpiamLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Opiam | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			OpiamLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			OpiamLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "OpiamOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, OpiamFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Opiam Settings', 0, true)

	if TabList['Opiam Settings'] then
		TabList['Opiam Settings'].LayoutOrder = 1000
	end

	if Elements['Opiam Settings'] then
		Elements['Opiam Settings'].LayoutOrder = 1000
	end
...
function OpiamLibrary:CreateWindow(Settings)
	if Opiam:FindFirstChild('Loading') then
		if getgenv and not getgenv().OpiamCached then
			Opiam.Enabled = true
			Opiam.Loading.Visible = true

			task.wait(1.4)
			Opiam.Loading.Visible = false
		end
	end

	if getgenv then getgenv().OpiamCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				OpiamLibrary:Notify({Title = 'Build Mismatch', Content = 'Opiam may encounter issues as you are running an incompatible interface version ('.. ((Opiam:FindFirstChild('Build') and Opiam.Build.Value) or 'No Build') ..').\n\nThis version of Opiam is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Opiam"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Opiam Interface Suite" then
		LoadingFrame.Version.Text = "Opiam UI"
	end
	...
	if not Settings.DisableOpiamPrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				OpiamLibrary:Notify({
					Title = "Opiam Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(OpiamFolder.."/Discord Invites")

		if callSafely(isfile, OpiamFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, OpiamFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Opiam RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(OpiamFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Opiam | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Opiam specific development.')
				end
			end
		end
		...
		if callSafely(isfile, OpiamFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, OpiamFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, OpiamFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						OpiamLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called OpiamLibrary:Destroy()
				if OpiamDestroyed then
					return
				end
				if not Success then
					...
					print("Opiam | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Opiam specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= OpiamLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					OpiamLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Opiam.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Opiam | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Opiam specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					OpiamLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Opiam | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Opiam specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					OpiamLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Opiam | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Opiam specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					OpiamLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= OpiamLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Opiam | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Opiam specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						OpiamLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Opiam | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Opiam specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					OpiamLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			OpiamLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			OpiamLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Opiam had an issue creating settings.') end
...
function OpiamLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function OpiamLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function OpiamLibrary:Destroy()
	OpiamDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Opiam Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "OpiamOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function OpiamLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				OpiamLibrary:Notify({Title = "Opiam Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			OpiamLibrary:Notify({Title = "Opiam Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Opiam Configurations Error | '..tostring(result))
			OpiamLibrary:Notify({Title = "Opiam Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = OpiamLibrary:CreateWindow({
	--	Name = "Opiam Example Window",
	--	LoadingTitle = "Opiam Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Opiam may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Opiam to get the key from
	...
	----OpiamLibrary:Notify({Title = "Opiam Interface", Content = "Welcome to Opiam. These - are the brand new notification design for Opiam, with custom sizing and Opiam calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(OpiamLibrary.Theme) do
...
task.delay(4, function()
	OpiamLibrary.LoadConfiguration()
